...
import {
	actGetProductReportsRequest,
	actGetRevenueRequest,
	actGetSalesRequest,
	actGetTotalRequest,
	actGetVisitorRequest,
	completeReport,
} from "../actions/AdminReportAction";
...
const AdminDashboard = () => {
	...
	useEffect(() => {
		if (Object.keys(data).length <= 5) {
			...
			if (lastKey === ReportKey.REPORTS_SALES)
				loadChartData(lastKey, salesData, setSalesData);
			else if (lastKey === ReportKey.REPORTS_REVENUE)
				loadChartData(lastKey, revenueData, setRevenueData);
			else if (lastKey === ReportKey.REPORTS_VISITOR)
				loadChartData(lastKey, visitorData, setVisitorData);
			else if (lastKey === ReportKey.REPORTS_PRODUCT)
				loadProductData(
					ReportKey.REPORTS_PRODUCT,
					productsData,
					setProductsData
				);
			else if (lastKey === ReportKey.REPORTS_TOTAL)
				loadTotalData(lastKey, totalData, setTotalData);
			// // console.log(data)
		} else {
	...
	const chartDataLoader = (charData, charData2) => {
		...
		if (charData2) {
			datasets.push({
				...
				backgroundColor: [charData2.color],
